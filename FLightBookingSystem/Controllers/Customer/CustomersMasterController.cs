...
namespace FLightBookingSystem.Controllers.Customer
{
    public class CustomersMasterController : Controller
    {
        ...
        public ActionResult CancelBooking()
        {
            ...
            var list = db.Bookings.Where(x => x.CustomerID == iduser && x.Status != "Canceled");
            ViewBag.BookingID = new SelectList(list, "BookingID", "BookingID");
            return View();
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult CancelBooking(Booking booking)
        {
            var userInCookie = Request.Cookies["UserInfo"];
            int iduser = Convert.ToInt32(userInCookie["idUser"]);
            if (ModelState.IsValid)
            {
                if (db.Flights.Any(x => x.DeptTime == booking.BookingDate))
                {
                    var lst = db.Bookings.FirstOrDefault(x => x.BookingID == booking.BookingID);
                    lst.BookingDate = booking.BookingDate;
                    lst.Status = "Canceled";
                    ViewBag.Message = "your Flight Canceld Money Tranfered to your Wallet" + lst.Price / 2;
                    db.SaveChanges();
                }
                var list1 = db.Bookings.Where(x => x.CustomerID == iduser && x.Status != "Canceled");
                if (!db.Flights.Any(x => x.DeptTime == booking.BookingDate))
                {
                    ViewBag.Message = "No any Booking found on this date";
                }
                else
                {
                    ViewBag.Message = "Flight Canceld Succesfully";
                }
                ViewBag.BookingID = new SelectList(list1, "BookingID", "BookingID");
                return View();
            }
            var list = db.Bookings.Where(x => x.CustomerID == iduser && x.Status != "Canceled");
            ViewBag.BookingID = new SelectList(list, "BookingID", "BookingID");
        ...
        public ActionResult ResheduleFlightBooking()
        {
            var userInCookie = Request.Cookies["UserInfo"];
            int iduser = Convert.ToInt32(userInCookie["idUser"]);
            if (iduser == 0)
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ResheduleFlightBooking([Bind(Include = "BookingID, BookingDate")] Booking booking)
        {
            var userInCookie = Request.Cookies["UserInfo"];
            int iduser = Convert.ToInt32(userInCookie["idUser"]);
            if (ModelState.IsValid)
            {
                if (db.Flights.Any(x => x.DeptTime == booking.BookingDate))
                {
                    var lst = db.Bookings.FirstOrDefault(x => x.BookingID == booking.BookingID);
                    lst.BookingDate = booking.BookingDate;
                    lst.Status = "Resheduled";
                    ViewBag.Message = "your Flight Resheduled";
                    db.SaveChanges();
                }
                var list1 = db.Bookings.Where(x => x.CustomerID == iduser);
                if (!db.Flights.Any(x => x.DeptTime == booking.BookingDate))
                {
                    ViewBag.Message = "Flight Not Available for this date";
                }
                else
                {
                    ViewBag.Message = "Flight Resheduled Succesfully";
                }
                ViewBag.BookingID = new SelectList(list1, "BookingID", "BookingID");
                return View();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ResheduleBooking(Booking booking)
        {
            var userInCookie = Request.Cookies["UserInfo"];
            int iduser = Convert.ToInt32(userInCookie["idUser"]);
            ...
            if (first != null)
            {
                ...
                var list = db.Bookings.Where(x => x.CustomerID == iduser && x.BookingDate == booking.BookingDate);
                ViewBag.BookingID = new SelectList(list, "BookingID", "BookingID");
                return View();
            }
            else
